...
def process_raw_path_to_vectors(path_log):
    ...
    while i < len(path_log) - 1:
        ...
        if action in ["forward", "backward"]:
            ...
            if abs(relative_angle) >= 5.0:  # Filtrer les micro-rotations
...
def generate_advanced_mowing_path(perimeter_vectors, mower_width_cm, concentric_passes=3):
    ...
    total_angle = sum(v.get("relative_angle", 0) for v in perimeter_vectors)
    print(f"DEBUG: Angle total du périmètre: {total_angle}°")
    
    # Une forme fermée doit avoir un angle total proche de ±360°
    if abs(abs(total_angle) - 360) > 45:
        print(f"ATTENTION: Forme probablement incomplète (angle total: {total_angle}°)")
    ...
    print(f"DEBUG: {len(polygon_points)} points générés")
    
    if len(polygon_points) < 3:
        print("ERREUR: Pas assez de points pour créer un polygone")
        return []
    
    # Forcer la fermeture du polygone si nécessaire
    ...
    try:
        ...
        if not main_polygon.is_valid:
            print("DEBUG: Polygone invalide, correction...")
            main_polygon = main_polygon.buffer(0)
        
        print(f"DEBUG: Aire du polygone: {main_polygon.area:.2f} cm²")
        
        all_waypoints = []
        
        # 1. PHASE PÉRIMÈTRE
        print("Phase 1: Tonte du périmètre...")
        perimeter_waypoints = _generate_perimeter_path(list(main_polygon.exterior.coords[:-1]), mower_width_cm)
        all_waypoints.extend(perimeter_waypoints)
        print(f"DEBUG: {len(perimeter_waypoints)} points de périmètre")
        
        # 2. PHASE CONCENTRIQUE
        print(f"Phase 2: Tonte concentrique ({concentric_passes} passes)...")
        current_polygon = main_polygon
        
        for pass_num in range(concentric_passes):
            inward_distance = mower_width_cm * (pass_num + 1)
            inner_polygon = current_polygon.buffer(-inward_distance)
            
            print(f"DEBUG: Pass {pass_num+1}, distance intérieure: {inward_distance}cm")
            
            if inner_polygon.is_empty:
                print(f"Zone vide après {pass_num} passes concentriques")
                break
                
            if inner_polygon.area < (mower_width_cm * mower_width_cm):
                print(f"Zone trop petite après {pass_num} passes concentriques")
                break
            ...
            if hasattr(inner_polygon, 'exterior') and inner_polygon.exterior:
                ...
                if len(inner_coords) >= 3:
                    ...
                    print(f"DEBUG: {len(concentric_waypoints)} points pour pass {pass_num+1}")
                else:
                    print(f"Pas assez de points pour pass {pass_num+1}")
                    break
            else:
                print(f"Pas d'extérieur valide pour pass {pass_num+1}")
                break
        
        # 3. PHASE PARALLÈLE du centre restant
        print("Phase 3: Tonte parallèle du centre...")
        if (not current_polygon.is_empty and 
            current_polygon.area > (mower_width_cm * mower_width_cm) and
            hasattr(current_polygon, 'bounds')):
            center_waypoints = _generate_parallel_path_for_polygon(current_polygon, mower_width_cm)
            all_waypoints.extend(center_waypoints)
            print(f"DEBUG: {len(center_waypoints)} points pour tonte parallèle")
        else:
            print("DEBUG: Pas de centre à tondre en parallèle")
        
        # Convertir en vecteurs
        final_vectors = _path_to_vectors(all_waypoints)
        print(f"RÉSULTAT: {len(all_waypoints)} waypoints → {len(final_vectors)} vecteurs")
...
def _vectors_to_polygon_points(vectors):
    """Convertit les vecteurs en liste de points (x, y)"""
    points = [(0, 0)]
    x, y, angle = 0, 0, -90
    
    for vector in vectors:
        angle += vector.get("relative_angle", 0)
...
def _generate_perimeter_path(polygon_points, mower_width_cm, reverse=False):
    """Génère le chemin de tonte du périmètre"""
    if len(polygon_points) < 3:
        return []
    
    waypoints = []
    points = polygon_points[::-1] if reverse else polygon_points
    
    offset_distance = mower_width_cm * 0.3
    ...
    for i in range(len(points)):
        ...
        # Calculer le vecteur normal vers l'intérieur
        ...
        if len_v1 > 0 and len_v2 > 0:
            n1 = (-v1[1]/len_v1, v1[0]/len_v1)
            n2 = (-v2[1]/len_v2, v2[0]/len_v2)
            normal = ((n1[0] + n2[0])/2, (n1[1] + n2[1])/2)
            
            offset_point = (
                p1[0] + normal[0] * offset_distance,
                p1[1] + normal[1] * offset_distance
            )
            waypoints.append({'x': offset_point[0], 'y': offset_point[1]})
        else:
            waypoints.append({'x': p1[0], 'y': p1[1]})
    
    return waypoints

def _generate_parallel_path_for_polygon(polygon, mower_width_cm):
    """Génère le chemin parallèle pour la zone centrale"""
    waypoints = []
    ...
    try:
        ...
        current_y = min_y + mower_width_cm / 2
        direction = 1
        
        while current_y < max_y:
            line_intersections = []
            
            line_start = (min_x - mower_width_cm, current_y)
            line_end = (max_x + mower_width_cm, current_y)
            
            scan_line = LineString([line_start, line_end])
            
            if polygon.intersects(scan_line):
                intersection = polygon.intersection(scan_line)
                
                if hasattr(intersection, 'coords'):
                    line_intersections.extend(intersection.coords)
                elif hasattr(intersection, 'geoms'):
                    for geom in intersection.geoms:
                        if hasattr(geom, 'coords'):
                            line_intersections.extend(geom.coords)
            
            line_intersections = sorted(set(line_intersections), key=lambda p: p[0])
            
            if len(line_intersections) >= 2:
                for i in range(0, len(line_intersections), 2):
                    if i + 1 < len(line_intersections):
                        x1, x2 = line_intersections[i][0], line_intersections[i + 1][0]
...
def generate_advanced_path_route(zone_name):
    """Génère un parcours de tonte avancé"""
    data = request.json
    ...
    advanced_path_vectors = generate_advanced_mowing_path(perimeter, float(mower_width), int(concentric_passes))
    
    if not advanced_path_vectors:
        return jsonify({"status": "error", "message": "Impossible de générer un parcours avancé."}), 500
    
    new_zone_name = f"{zone_name}_parcours_avance"
    if db_manager.save_zone(new_zone_name, advanced_path_vectors):
        return jsonify({
            "status": "success", 
            "message": f"Parcours avancé '{new_zone_name}' généré.",
            "details": f"Périmètre + {concentric_passes} passes concentriques + parallèle"
        })
    else:
        return jsonify({"status": "error", "message": f"Le parcours '{new_zone_name}' existe déjà."}), 409
